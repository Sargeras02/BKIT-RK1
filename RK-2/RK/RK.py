...
def task1():
    print('Задание Д1')
    relations = getRelations_OtO()
    res_d1 = list(filter(lambda x: x[0][len(x[0])-2:]=='ов', relations))
    print(res_d1)
    return res_d1
...
def task2():
    ...
    for os in OSs:
        ...
        compsWithOS = list(filter(lambda x: x[2]==os.name, relations))
